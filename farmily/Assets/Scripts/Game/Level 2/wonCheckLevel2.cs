...
public class wonCheckLevel2 : MonoBehaviour
{
    ...
    public collectorScript coll;
    public objectivePanelLevel2 objective;
    ...
    void Update()
    {
        ket = PlayerPrefs.GetString("Level 2");
        if (objectiveDone == totalObjective && finished == false)
        {
            ...
            if (time.timerMinute == 0 && time.timerSecond <= 40 && ket == "Level 2")
            {
                goldMedal = PlayerPrefs.GetInt("Gold Medal");
                PlayerPrefs.SetInt("Gold Medal", goldMedal + 1);
            }
            else if (time.timerMinute == 0 && 40 < time.timerSecond && time.timerSecond <= 60 && ket == "Level 2" ||
                     time.timerMinute == 1 && 0 <= time.timerSecond && time.timerSecond < 30 && ket == "Level 2")
            {
                silverMedal = PlayerPrefs.GetInt("Silver Medal");
                PlayerPrefs.SetInt("Silver Medal", silverMedal + 1);
            }
            else if (time.timerMinute == 1 && 30 < time.timerSecond && time.timerSecond <= 60 && ket == "Level 2" ||
                     time.timerMinute == 2 && 0 <= time.timerSecond && time.timerSecond <= 10 && ket == "Level 2")
            {
                bronzeMedal = PlayerPrefs.GetInt("Bronze Medal");
                PlayerPrefs.SetInt("Bronze Medal", bronzeMedal + 1);
            }

            PlayerPrefs.SetString("Level 2", "Level 2 is done");
            finished = true;
        }
